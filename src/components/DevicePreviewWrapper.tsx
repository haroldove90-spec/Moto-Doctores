...
export function DevicePreviewWrapper({ children }: DevicePreviewWrapperProps) {
  ...
  return (
    <div className="min-h-screen bg-motodoctores-background overflow-hidden flex flex-col">
      {/* Control Toggle */}
      <div className="fixed top-20 right-4 z-[10002]">
        <button 
          onClick={() => setShowControls(!showControls)}
          className="glass-panel text-motodoctores-text p-3 rounded-full border-motodoctores-accent/30 hover:bg-motodoctores-accent/20 transition-all shadow-2xl group"
        >
          <Zap className={cn("w-5 h-5 text-motodoctores-accent", showControls && "animate-pulse")} />
        </button>
      </div>
      ...
      <AnimatePresence>
        {showControls && (
          <motion.div
            ...
            className="fixed top-36 right-4 z-[10002] w-64 glass-panel p-6 rounded-3xl border-motodoctores-accent/30 shadow-[0_20px_50px_rgba(0,0,0,0.5)]"
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-motodoctores-accent/70">AUDITORÍA RESPONSIVA</h3>
              <button onClick={() => setShowControls(false)} className="btn-reset text-motodoctores-text/40 hover:text-motodoctores-text">
                <X className="w-4 h-4" />
              </button>
            </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      device === key 
                        ? "bg-motodoctores-accent/20 border-motodoctores-accent/50 text-motodoctores-accent" 
                        : "bg-motodoctores-background/50 border-white/5 text-motodoctores-text/40 hover:border-white/20"
                    )}
                  >
            ...
            <div className="mt-8 pt-6 border-t border-white/5">
              <p className="text-[8px] text-motodoctores-text/20 uppercase tracking-widest leading-relaxed">
      ...
      <div className="flex-1 flex items-center justify-center p-4 md:p-10 overflow-auto">
        <motion.div
          ...
          className={cn(
            "bg-motodoctores-background relative shadow-[0_50px_100px_rgba(0,0,0,0.8)] overflow-hidden border-white/10",
