...
export function PatientDashboard() {
  ...
  return (
    <div className="min-h-screen bg-brand-navy text-brand-white pb-24 cyber-grid">
      ...
      {/* Header */}
      <header className="sticky top-0 z-50 glass-panel px-6 py-4 flex justify-between items-center border-b border-brand-blue/20">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-brand-blue/20 rounded-lg flex items-center justify-center border border-brand-blue/50 shadow-[0_0_10px_rgba(0,82,155,0.3)]">
        ...
        <div className="flex items-center gap-4">
          <div className="hidden md:block text-right">
            ...
            <p className="text-sm font-mono text-brand-blue">V.2.0.4</p>
          </div>
          <div className="w-10 h-10 rounded-full bg-slate-800 border border-brand-blue/30 flex items-center justify-center text-brand-blue font-bold shadow-inner">
            JD
          </div>
        </div>
      </header>
        ...
        {view === 'home' ? (
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-1/3 space-y-6 order-2 lg:order-1">
              <section className="glass-panel rounded-3xl p-6 relative overflow-hidden group">
                ...
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-brand-blue/70">BIOMETRÍA</h3>
                  <div className="flex gap-1">
                    <span className="w-1 h-1 bg-brand-blue rounded-full animate-pulse"></span>
                    <span className="w-1 h-1 bg-brand-blue rounded-full animate-pulse delay-75"></span>
                    <span className="w-1 h-1 bg-brand-blue rounded-full animate-pulse delay-150"></span>
                  </div>
                </div>
                ...
                <div className="flex justify-center py-4 relative">
                  ...
                  <div className="relative w-48 h-64 opacity-40">
                    <svg viewBox="0 0 200 400" className="w-full h-full fill-brand-blue/20 stroke-brand-blue/40 stroke-1">
                    ...
                    <motion.div 
                      ...
                      className="absolute left-0 w-full h-[2px] bg-brand-blue shadow-[0_0_10px_rgba(0,82,155,0.8)] z-10"
                    />
                  </div>
                  ...
                  <div className="absolute top-4 right-0 flex flex-col items-end gap-4">
                    <div className="text-right">
                      ...
                      <p className="text-lg font-mono font-bold text-brand-blue">72 <span className="text-[10px]">BPM</span></p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] text-slate-500 uppercase">Temp</p>
                      <p className="text-lg font-mono font-bold text-brand-blue">36.6 <span className="text-[10px]">°C</span></p>
                    </div>
                  </div>
                  <div className="absolute bottom-4 left-0 flex flex-col items-start gap-4">
                    <div className="text-left">
                      ...
                      <p className="text-lg font-mono font-bold text-brand-blue">98 <span className="text-[10px]">%</span></p>
                    </div>
                  </div>
                </div>
              </section>
            ...
            <div className="lg:col-span-8 space-y-6">
              {/* Map Section */}
              <section className="glass-panel rounded-3xl overflow-hidden h-[400px] relative border border-brand-blue/20">
                <TrackingMap 
                ...
                <div className="absolute top-4 left-4 z-10">
                  <div className="glass-panel px-4 py-2 rounded-full flex items-center gap-2 border-brand-blue/30">
                    <div className="w-2 h-2 bg-brand-blue rounded-full animate-pulse"></div>
                    <span className="text-[10px] font-mono text-brand-blue uppercase tracking-widest">GPS ACTIVE</span>
                  </div>
                </div>
                <div className="absolute bottom-4 left-4 right-4 bg-slate-900/80 backdrop-blur-md p-4 rounded-2xl border border-white/10 flex items-center gap-4 z-10">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-medium truncate text-brand-white">
                ...
                {activeAppointment ? (
                  ...
                  >
                    <div className="absolute top-0 right-0 p-4">
                      <Zap className="w-6 h-6 text-brand-blue/20" />
                    </div>
                    <div className="flex flex-col md:flex-row justify-between gap-6">
                      <div className="space-y-4">
                        <div>
                          <span className="inline-block px-3 py-1 rounded-full bg-brand-blue/10 text-brand-blue text-[10px] font-bold uppercase tracking-[0.2em] border border-brand-blue/30 mb-2">
                        ...
                        {activeAppointment.status === 'en_camino' && (
                          <div className="flex items-center gap-6">
                            <div className="p-4 bg-slate-800/50 rounded-2xl border border-white/5">
                              ...
                              <p className="text-2xl font-mono font-bold text-brand-blue">{activeAppointment.eta}</p>
                            </div>
                            <div className="flex gap-3">
                              <button className="w-12 h-12 rounded-2xl bg-brand-blue text-brand-white flex items-center justify-center shadow-[0_0_15px_rgba(0,82,155,0.4)] hover:scale-105 transition-transform">
                  ...
                  >
                    {isRequesting ? (
                      ...
                      <>
                        <div className="w-12 h-12 bg-brand-blue rounded-2xl flex items-center justify-center text-brand-white shadow-[0_0_20px_rgba(0,82,155,0.5)] group-hover:scale-110 transition-transform">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { label: 'CORAZÓN', value: '84%', icon: Heart, color: 'text-brand-blue' },
                  { label: 'OXÍGENO', value: '99%', icon: Activity, color: 'text-brand-blue' },
                  { label: 'HIDRATACIÓN', value: '62%', icon: Zap, color: 'text-brand-yellow' },
                  { label: 'TEMP', value: '36.8°', icon: Thermometer, color: 'text-brand-yellow' }
                ].map((stat, idx) => (
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 glass-panel h-16 rounded-full px-8 flex items-center gap-12 z-50 border-brand-blue/20">
        <button 
          onClick={() => setView('home')}
          className={cn("p-2 transition-all duration-300", view === 'home' ? "text-brand-blue scale-125 drop-shadow-[0_0_8px_rgba(0,82,155,0.6)]" : "text-slate-500 hover:text-slate-300")}
        >
          <Activity className="w-6 h-6" />
        </button>
        <button 
          onClick={() => setView('history')}
          className={cn("p-2 transition-all duration-300", view === 'history' ? "text-brand-blue scale-125 drop-shadow-[0_0_8px_rgba(0,82,155,0.6)]" : "text-slate-500 hover:text-slate-300")}
        >
